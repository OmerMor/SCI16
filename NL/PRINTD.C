...
char*   message;          /* global string pointer, use for printing
   ...
   char stmp[20];
	...
	if (getnodetype(np)==wordtype)
	{
	   if (np->data.token >= 0x1000)
	   {
	      ...
	      /*
	      ift[SPRINTF].function(stmp,"x%x",np->data.token);
	      */
	   }
	   else
